...
const fieldStyles = {
  ...
  desc: { opacity: 0.8, marginTop: 2 },
  control: {},
  input: { width: "100%", padding: 8 },
  numberControlWrap: {
    display: "flex",
    justifyContent: "flex-end",   // aligns nicely on the right
  },
  numberInput: {
    width: 220,                   // looks intentional
    maxWidth: "100%",
    padding: 8,
  }
};
...
function Field({ schema, def, path, value, onChange, layout, cols  }) {
...
if (def?.type === "number" || def?.type === "integer") {
 ...
 if (layout === "scanner") {
  const rowStyle = {
    display: "grid",
    gridTemplateColumns: cols >= 2 ? "1fr 260px" : "1fr",
    gap: 10,
    alignItems: cols >= 2 ? "center" : "stretch",
    paddingBottom: 12,
    marginBottom: 12,
    borderBottom: "1px solid #eee",
  };
  ...
  const inputStyle = {
    width: "100%",          // fill the 260px column
    padding: 8,
  };
  ...
  return (
    <div style={rowStyle}>
      <div>
        <div style={{ fontWeight: 600 }}>{title}</div>
        {desc ? <div style={{ opacity: 0.8, marginTop: 2 }}>{desc}</div> : null}
      </div>
...
function renderObjectBlock(blockTitle, blockPath, blockDef,options = {}) {
  ...
  return (
    <div style={styles.page}>
      ...
      >
        ...
        {renderObjectBlock("Scanner", ["scanner"], scannerDef, { layout: "scanner" })}
